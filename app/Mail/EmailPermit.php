...
class EmailPermit extends Mailable implements ShouldQueue
{
    ...
    public function build()
    {
		...
		$body = '<div style="margin-bottom:15px;">Dear Customer,</div>
				<div style="margin-bottom:5px;">We have received your item(s). Please execute the exchange by replacing a new order using this Voucher Code:</div>
				<div style="margin-bottom:35px;text-align:center;font-size:15px;"><span style="background-color:#723879;color:white;padding: 5px 15px;">'.$this->data->orderpermit->voucher.'</span></div>
				<div style="margin-bottom:15px;">We do not guarantee the availability of our products. A new order does not include shipping costs, taxes and convenience fee. You only can order for the same amount or more, to get the item that you wish to receive in exchange.</div>
				<div style="margin-bottom:15px;">Hope you enjoy shopping with us.</div>		
		...
		$action = '<div style="border-top: 1px solid #b69bbc; padding-top: 30px; margin: 30px 0;">
					...
					<div style="margin-bottom: 15px;">
						...
						<div style="display:inline-block; width: 190px; font : 300 14px/18px \'Lucida Grande\', Lucida Sans, Lucida Sans Unicode, sans-serif, Arial, Helvetica, Verdana, sans-serif; font-size: 14px; color: #3971ff; line-height: 25px; font-weight: bold;">EXCHANGED</div>
					</div>';
		...
		foreach($this->data->orderpermit->permitdetail as $orderdetail){
		...
		return $this->view('email/defaultorder')->subject("Demor Boutique : Your order has been permitted for exchange")->with("data",$payment);
    }
}
