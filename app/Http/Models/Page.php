...
class Page
{
	...
	public function updatePopup($method,$link,$image,$message,$start_popup,$end_popup,$is_active)
	{
		//dd(date("Y-m-d",strtotime($end_popup)),$end_popup);
		DB::update('update popup set popup_type = ?, link_path = ?, image_path = ?, message = ?, start_popup = ?, end_popup = ?, is_active = ?', [$method,$link,$image,$message,$start_popup,$end_popup,$is_active]);
	}
	...
	{
		return DB::select('select * from pages_slider order by sliderid desc');
	}
	public function getAllActiveSlider()
	{
		return DB::select('select * from pages_slider where ispublish = 1 order by sortby asc');
	}
	public function addSlider($path, $extension, $isPublish)
	{
		//2019-01-07 penambahan sortby
		DB::update('update pages_slider set sortby = sortby + 1');
		DB::insert('insert into pages_slider(sliderpath, imagetype, uploaddate, ispublish, sortby) values(?, ?, NOW(), ?, 1)', [$path, $extension, $isPublish]);
	}
	public function editSlider($id, $isPublish, $sortBy)
	{
		//2019-01-07 penambahan sortby
		DB::update('update pages_slider set ispublish = ?, sortby = ? where sliderid = ?', [$isPublish, $sortBy, $id]);
	}
	//2019-01-07 penambahan sortby
	public function checkUniqueSortBy($id,$sortBy)
	{
		return DB::select('select * from pages_slider where sliderid <> ? and sortby = ?', [$id,$sortBy]);
	}
	...
	{
		return DB::select('select a.blogid, name, method, description, urlpath, filepath, youtubepath
							from blog_list a join blog_list_image b on a.blogid = b.listid
							and filepath = (select filepath from blog_list_image where listid = a.blogid limit 1) order by a.blogid desc');
	}
	public function getCategoryDetailBlogList($id)
	{
		return DB::select('select a.blogid, name, method, description, urlpath, filepath, youtubepath
							...
							where a.categoryid = ? order by a.blogid desc', [$id]);
	}
	...
	public function addBlogList($blogName, $categoryId, $methods, $url, $subtitle, $image, $youtubePath="")
	{
		...
		if($youtubePath == ""){
			...
			DB::insert('insert into blog_list_image values(?, ?, ?, ?)', [$id, "", "",$youtubePath]);
		}
	}
	public function editBlogList($blogId, $blogName, $categoryId, $methods, $url, $subtitle, $image, $youtubePath="")
	{
		...
		if(count($image) != 0){
			...
			for($i = 0; $i < count($image); $i++){
				...
				if(is_array($url))
					...
					$tempUrl = $url;
				DB::insert('insert into blog_list_image values(?, ?, ?, ?)', [$blogId, $image[$i], $tempUrl, $youtubePath]);
			}
		}
		else{
		...
		if($youtubePath!=""){
			DB::update('update blog_list_image set filepath = ?, youtubepath = ? where listid = ?', ["",$youtubePath, $blogId]);
		}
	}
