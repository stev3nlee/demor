				...
				<div class="col-sm-10 col-sm-offset-1">
					<div class="row">
						<div class="col-xs-6">
							@if($others->method == 1)
									<div class="item">
									<a href="{{ url('product/categories/'.$others->categoryid)}}">
@if($others->withbanner == 1)
<img src="{{ url($others->categorybanner) }}" class="img-responsive"/></a>
@else
<img src="{{ url($others->collectionbanner) }}" class="img-responsive"/></a>
@endif
									</div>
							...
							<div class="index-slider owl-carousel">
								@foreach($populars as $popular)
									<div class="item">
									<a href="{{ url('product/detail/'.$popular->productid) }}"><img src="{{ url($popular->mainimage) }}?v.1" class="img-responsive"/></a>
									</div>
								@endforeach
							</div>
							<div class="t-index">{{$others->submenuname}}</div>
							...
							<a href="{{ url('product/arrival') }}">
								<div class="index-slider owl-carousel">
									@foreach($arrivals as $arrival)
										<div class="item">
											<img src="{{ url($arrival->mainimage) }}?v.1" class="img-responsive"/>
										</div>
									@endforeach
								</div>
	...
	@endphp
	@if(strtotime(date("Y-m-d"))>=strtotime($popup->start_popup) and strtotime(date("Y-m-d"))<=strtotime($popup->end_popup) and Session::get('popup')['is_popped'] == 0 and $popup->is_active == 1)
	<script>
