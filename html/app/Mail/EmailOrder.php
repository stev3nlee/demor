...
class EmailOrder extends Mailable implements ShouldQueue
{
  ...
  protected $payments;
    ...
    public function build()
    {
		...
		if($this->data->orderheader[0]->status == 'Pending' && $this->data->orderheader[0]->paymenttype == 'Bank Transfer')
		{
			$body = '<div style="margin-bottom:15px;">Dear Customer,</div>
					<div>Please do the payment via bank transfer in the next 24 hours as follows:</div>';
          $x=0;
          foreach($this->payments as $payment)
          {
            if($x > 0){ $body .='<div>Or</div>'; }
            $body .='<div>';
            $body .='<div>'.$payment->bankname.'</div>';
            $body .='<div>'.$payment->accountnumber.' A/N '.$payment->bankaccountname.'</div>';
            $body .='</div>';
            $x++;
          }
			$body .='<div style="margin-bottom:15px;">Afther which we will realese the order. Please confirm the payment in the <a style="color:#703879; text-decoration:none;font-weight:bold;"href="'.url('member/confirmpayment').'">Member Area.</a></div>
		...
		else if($this->data->orderheader[0]->status == 'Waiting' && $this->data->orderheader[0]->paymenttype == 'Bank Transfer')
		{
			$body = '<div style="margin-bottom:15px;">Dear Customer,</div>
		...
		else if($this->data->orderheader[0]->status == 'Paid' && $this->data->orderheader[0]->paymenttype == 'Bank Transfer')
		{
			$body = '<div style="margin-bottom:15px;">Dear Customer,</div>
		...
		else if($this->data->orderheader[0]->status == 'Paid' && $this->data->orderheader[0]->paymenttype == 'VT Web')
		{
			$body = '<div style="margin-bottom:15px;">Dear Customer,</div>
		...
		else if($this->data->orderheader[0]->status == 'Ship' && $this->data->orderheader[0]->paymenttype == 'Bank Transfer')
		{
			$body = '<div style="margin-bottom:15px;">Dear Customer,</div>
		...
		else if($this->data->orderheader[0]->status == 'Ship' && $this->data->orderheader[0]->paymenttype == 'VT Web')
		{
			$body = '<div style="margin-bottom:15px;">Dear Customer,</div>
					<div>Your order has been shipped.</div>
					<div style="margin-bottom:15px;">Please share your pictures of you "being fashionable and glamorous" with De\'mor design and please do not forget to tag <span style="color:#703879;font-weight:bold;">@demorboutique</span> in your Facebook and Instagram pictures.</div>
		...
		else if($this->data->orderheader[0]->status == 'Cancel' && $this->data->orderheader[0]->paymenttype == 'Bank Transfer')
		{
			$body = '<div style="margin-bottom:15px;">Dear Customer,</div>
						<div>Your order has been cancelled.</div>
						<div style="margin-bottom:15px;">You can place a new order through our website for the same product or other products.</div>
						<div>These are the following details of your order: </div>';
			...
			$action2 .= '<div style="margin-bottom: 30px; padding-bottom: 30px; border-bottom: 1px solid #c6c6c6;">
							...
							<div style="font : 300 14px/18px \'Lucida Grande\', Lucida Sans, Lucida Sans Unicode, sans-serif, Arial, Helvetica, Verdana, sans-serif; font-size: 13px; color: #757575; margin-bottom: 5px;">
								...
								<div style="display: inline-block; width: 100px;">
									<img src="'.url($orderdetail->productcolor).'" style="width: 12px; position: relative; top: 3px;"/>
								</div>
							</div>
							<div style="font : 300 14px/18px \'Lucida Grande\', Lucida Sans, Lucida Sans Unicode, sans-serif, Arial, Helvetica, Verdana, sans-serif; font-size: 13px; color: #757575; margin-bottom: 5px;">
		...
		return $this->view('email/defaultorder')->subject("Demor Boutique : Thank you for your order")->with("data",$payment);
    }
}
