...
class AdminPagesController extends Controller
{
	...
	public function submitBlogList(Request $request)
	{
		...
		else if($methods == 'select-video')
		{
			...
			$page->addBlogList($blogName, $categoryId, $methods, '', $subtitleVideo, $videoPath);
		}
	...
	public function submitEditBlogList(Request $request)
	{
		...
		else if($methods == 'select-video')
		{
			...
			$page->editBlogList($blogId, $blogName, $categoryId, $methods, '', $subtitleVideo, $videoPath);
		}
	...
	public function sendnewsletter(Request $request)
	{
		...
		if($request->input('to') == 3){
			$o = new Order;
			$emails=$o->getMemberEmail();
			Mail::to('dwj.rian@gmail.com')->send(new EmailNewsletter($request->input('campaign_name'),$request->input('template'),$products,$pageImage));
		}
