...
class Product
{
	...
	{
		return DB::select('select a.productid, productcode, productname, brandname, a.categoryid, categoryname, price, discount, productdescription, sizechart, null as color, 0 as stock
							from product a
							join product_category b on a.categoryid = b.categoryid
	...
	public function submitProduct($category, $productCode, $productName, $brandName, $price, $sale, $productDescription, $sizeChart, $sizeSale, $colourImage, $genStock, $genImage)
	{
		DB::insert('insert into product(categoryid, productcode, productname, brandname, price, discount, productdescription, sizechart, insertdate, updatedate, isdeleted) values(?, ?, ?, ?, ?, ?, ?, ?, now(), now(), 1)',
						[$category, $productCode, $productName, $brandName, $price, $sale, $productDescription, $sizeChart]);
	...
	public function editProduct($productId, $reGenerate, $genSize, $category, $productCode, $productName, $brandName, $price, $sale, $productDescription, $sizeChart, $sizeSale, $colourImage, $genStock, $genImage)
	{
		DB::update('update product set categoryid = ?, productname = ?, brandname = ?, price = ?, discount = ?, productdescription = ?, sizechart = ?, updatedate = now() where productid = ?',
						[$category, $productName, $brandName, $price, $sale, $productDescription, $sizeChart, $productId]);
		if($reGenerate == "true"){
