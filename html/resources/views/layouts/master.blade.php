...
<head>
	...
	if(selectedCurrency == null){
	  ...
	  $.ajax({
			type : "get",
			url  : "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.xchange%20where%20pair%20in%20({{$sqlCurrency}})&format=json&diagnostics=true&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&callback=",
			success: function (e){
				a=e.query.results.rate;
				sessionStorage.setItem("listCurrency",JSON.stringify(a));
				for (var i = 0; i < a.length; i++){
					$('.select-currency').append('<option value="'+a[i].Name.substring(0,3)+'" data-value="'+a[i].Rate+'">'+a[i].Name.substring(0,3)+'</option>');
				}
		...
		a = $.parseJSON(sessionStorage.getItem("listCurrency"));
		$(function() {
			for (var i = 0; i < a.length; i++){
				$('.select-currency').append('<option value="'+a[i].Name.substring(0,3)+'" data-value="'+a[i].Rate+'">'+a[i].Name.substring(0,3)+'</option>');
			}
