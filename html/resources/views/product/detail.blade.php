					...
					<div class="pos-rel">
						...
						<ul class="zoomslider">
							<li>
								<div class="imagezoom">
									@foreach($product->color as $index => $color)
										<img src="{{ url($color->mainimage) }}" class="tab-{{$index+1}} content-img @if($index == 0) current @endif img-responsive" />
									@endforeach
								</div>
							</li>
							<li>
								<div class="imagezoom">
									@foreach($product->color as $index => $color)
										<img src="{{ url($color->subimage) }}" class="tab-{{$index+1}} content-img @if($index == 0) current @endif img-responsive" />
									@endforeach
								</div>
							</li>
							@for($i = 0; $i < $subImageCount; $i++)
								<li>
									<div class="imagezoom">
										@for($j = 0; $j < count($product->color); $j++)
											<img src="{{ url($product->color[$j]->image[$i]->subimage) }}" class="tab-{{$j+1}} content-img @if($j == 0) current @endif img-responsive" />
										@endfor
									</div>
								</li>
							@endfor
